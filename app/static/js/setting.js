...
async function handleSaveScript(event){
    ...
    const logFilePath=document.getElementById('logpath').value
    const serverInstance = document.getElementById('serverInstance').value;
    const response = await fetch('/save_script',{
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({script: script,logFilePath:logFilePath,serverInstance:serverInstance}),
    })
...
async function handleOnchangeInstance(event){

    console.log(event.target.value)
    const response =await fetch('/getSettingDetails/'+event.target.value)
    const data=await response.json()
    console.log(data)
    document.getElementById('script').value=data.script
    document.getElementById('logpath').value=data.logfilepath
}